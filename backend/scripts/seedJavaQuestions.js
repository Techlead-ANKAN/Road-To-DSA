...
const main = async () => {
  try {
    console.log('🚀 Starting Java Questions Seeding Process...\n')
    ...
    const questionsPath = path.join(__dirname, 'javaQuestions.json')
    ...
    const existingCount = await InterviewQuestion.countDocuments({ subject: 'JAVA' })
    console.log(`📊 Existing JAVA questions in database: ${existingCount}`)

    if (existingCount > 0) {
      console.log('\n⚠️  Warning: JAVA questions already exist in the database.')
      ...
      console.log('\n💡 Run with argument: node seedJavaQuestions.js [replace|skip|merge]')
      
      const mode = process.argv[2] || 'replace'
      console.log(`\n🔧 Mode selected: ${mode}`)

      if (mode === 'replace') {
        console.log('\n🗑️  Deleting existing JAVA questions...')
        const deleteResult = await InterviewQuestion.deleteMany({ subject: 'JAVA' })
        ...
        const existingOrders = await InterviewQuestion.distinct('order', { subject: 'JAVA' })
        ...
        console.log(`   - Total JAVA questions: ${existingCount + insertResult.length}`)
    ...
    // Verify insertion
    const finalCount = await InterviewQuestion.countDocuments({ subject: 'JAVA' })
    console.log(`\n📊 Total JAVA questions in database: ${finalCount}`)

    // Sample query to verify
    const sampleQuestions = await InterviewQuestion.find({ subject: 'JAVA' })
      .sort({ order: 1 })
      .limit(3)
    ...
    console.log('\n💡 Usage examples:')
    console.log('   - View all: GET /api/interview-questions?subject=JAVA')
    console.log('   - Filter by tag: GET /api/interview-questions?subject=JAVA&tags=OOP')
